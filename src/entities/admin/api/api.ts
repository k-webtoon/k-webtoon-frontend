...
export const adminApi = {
  ...
  updateWebtoon: async (webtoonId: any, webtoonData: any) => {
    const response = await axios.put(
      `${API_BASE_URL}/admin/webtoons/${webtoonId}`,
      webtoonData,
      {
        withCredentials: true,
      }
  ...
  // 사용자 전체 조회 api
  getAlluser: async (page: number = 0, size: number = 10) => {
    const response = await axios.get(
  ...
  ): Promise<AccountStatusRequest> => {
    const response = await axios.patch(
      `${API_BASE_URL}/user_ma/supend`,
      request,
      { withCredentials: true }
    );
    return response.data as AccountStatusRequest;
  },
};
