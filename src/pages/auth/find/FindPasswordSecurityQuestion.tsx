...
const FindPasswordSecurityQuestionPage: React.FC = () => {
  ...
  // URL에서 데이터 가져오기
  const { email, phoneNumber, question } = location.state || {};
        ...
        <div className="container max-w-screen-xl mx-auto px-4">
          <div className="max-w-md mx-auto text-center py-20">
            ...
            <p className="text-muted-foreground mb-6">
              이메일과 전화번호 인증을 먼저 진행해주세요.
            </p>
            <Button asChild>
              <Link to="/auth/find/password">돌아가기</Link>
            </Button>
          </div>
        </div>
      </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 비밀번호 찾기용 보안 질문 답변 검증 - findEmail API 사용
...
export default FindPasswordSecurityQuestionPage; 