...
import { UserCircle2 } from "lucide-react"; // 아이콘 임포트 추가

interface FollowersListProps {
  followers: FollowUser[];
}

const FollowersList: React.FC<FollowersListProps> = React.memo(
  ({ followers }) => {
    const userIds = useMemo(() => followers.map((f) => f.indexId), [followers]);
              ...
              <CardContent className="p-4">
                <div className="flex items-center space-x-4">
                  ...
                  {profileImages[follower.indexId] ? (
                    <img
                      src={profileImages[follower.indexId] ?? undefined}
