...
const UserFollowees = () => {
  ...
  const { userInfo, followees, loading, error, fetchUserInfo, fetchFollowees } = useUserStore();
    ...
    <div className="container mx-auto px-4 py-8 mt-16 max-w-7xl">
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="border-b pb-4 mb-6">
          ...
          <p className="text-gray-500 mt-1">총 {followees?.length || 0}명</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {followees?.map((followee) => (
            <Link to={`/user/${followee.indexId}`} key={followee.indexId}>
          ...
          {(!followees || followees.length === 0) && (
            <div className="col-span-full flex flex-col items-center justify-center py-12 text-gray-500">
              ...
              <p className="text-lg font-medium">팔로우하는 사용자가 없습니다.</p>
              <p className="text-sm">다른 사용자를 팔로우해보세요!</p>
            </div>
          )}
        </div>
      </div>
