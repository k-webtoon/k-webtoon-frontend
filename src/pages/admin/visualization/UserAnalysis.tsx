...
import {
  fetchPcaData,
  fetchAvgScatterData,
} from "@/entities/admin-visualization/api/api";
...
const UserAnalysis: FC = () => {
  ...
  const scatterPcaOptions: ChartOptions<"scatter"> = {
    ...
    scales: {
      x: {
        title: { display: true, text: "PC1", font: { size: 18 } },
        ticks: { font: { size: 14 } },
      },
      y: {
        title: { display: true, text: "PC2", font: { size: 18 } },
        ticks: { font: { size: 14 } },
      },
    },
  };
  ...
  const scatterOtherChartOptions: ChartOptions<"scatter"> = {
    ...
    scales: {
      x: {
        title: { display: true, text: "평균 유사도", font: { size: 18 } },
        ticks: { font: { size: 14 } },
      },
      y: {
        title: { display: true, text: "평균 평점", font: { size: 18 } },
        ticks: { font: { size: 14 } },
      },
    },
  };
  ...
  const handleOtherDataClick = async () =>
    setScatterDataOther(await fetchAvgScatterData());
        ...
        <CardContent className="p-6">
          ...
          <div className="h-[600px] bg-white rounded-lg">
            {scatterDataOther.length > 0 ? (
              <Scatter
                data={scatterOtherChartData}
                options={scatterOtherChartOptions}
              />
            ) : (
